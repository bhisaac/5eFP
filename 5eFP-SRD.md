...
- Defender, Damage, and Healing group roles are used when forming a group. The design goal for group roles is to allow any character to gain the basic tools for any of the three standard game strategies: turtle (hunker down and absorb hits, allowing your teammates to deal with an encounter while it is distracted by you), HOT (heal over time), and DOT (damage over time).
